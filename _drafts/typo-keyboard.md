...
| `[Compose]→[a]→[e]`     | `æ`  | `U+00E6 LATIN SMALL LETTER AE` | Аналогично для заглавных. Так же работает для некоторых распространенных лигатур, например `ﬁ`, но не для всех. |
...
Выше я писал, что обнаружил баг на своей системе, и сейчас его обхожу. Дело в том, что сам «типографский» набор вводит неразрывный пробел
...
Предустановленные комбинации Compose, как выше уже упоминалось, в файле `/usr/share/X11/locale/en_US.UTF-8/Compose`.

<!-- TODO: внести в картинку квадратные и фигурные скобки -->
